...
void corrMap::loadCorrMaps(const std::string &file_accposneg3D, const std::string &file_ptcorr2D, const std::string &file_ringcorr2D, const std::string &file_etacorr1D)
{
    TFile *f_accposneg3D = new TFile(file_accposneg3D.c_str(), "READ");
    accpos3D = (TH3D *)f_accposneg3D->Get("accpos3D");
    accneg3D = (TH3D *)f_accposneg3D->Get("accneg3D");
    accneg3D->SetDirectory(0);
    accpos3D->SetDirectory(0);
    f_accposneg3D->Close();

    TFile *f_ptcorr2D = new TFile(file_ptcorr2D.c_str(), "READ");
    ptcorr2D = (TH2F *)f_ptcorr2D->Get("ptcorr2D");
    ptcorr2D->SetDirectory(0);
    f_ptcorr2D->Close();

    TFile *f_ringcorr2D = new TFile(file_ringcorr2D.c_str(), "READ");
    ringcorr2D = (TH2F *)f_ringcorr2D->Get("ptcorr2D");
    ringcorr2D->SetDirectory(0);
    f_ringcorr2D->Close();

    TFile *f_etacorr1D = new TFile(file_etacorr1D.c_str(), "READ");
    etacorr1D = (TH2F *)f_etacorr1D->Get("etacorr1D");
    etacorr1D->SetDirectory(0);
    f_etacorr1D->Close();
}

std::tuple<float, float, float> corrMap::getCorrections(const float &pt, const float &eta, const float &phi, const int &charge) const
{
    // guard
    if (!accpos3D || !accneg3D || !ptcorr2D || !ringcorr2D || !etacorr1D)
    {
        if (verbosity > 0)
            std::cout << "Correction histograms are neither set nor loaded. Set corrections to 1 (corrections are NOT applied)" << std::endl;
    ...
    int scalebineta = ptcorr2D->GetXaxis()->FindBin(eta);
    int scalebinphi = ptcorr2D->GetYaxis()->FindBin(phi);
    int scalebin = ptcorr2D->GetBin(scalebineta, scalebinphi);
    scalecorr = ptcorr2D->GetBinContent(scalebin);
    ringcorr = ringcorr2D->GetBinContent(scalebin);
    int etacorrbin = etacorr1D->GetXaxis()->FindBin(eta);
    etacorr = etacorr1D->GetBinContent(etacorrbin);
    int binpt = accpos3D->GetXaxis()->FindBin(1.0 / pt);
    int bineta = accpos3D->GetYaxis()->FindBin(eta);
    int binphi = accpos3D->GetZaxis()->FindBin(phi);

    float acccorr = 0;
    if (charge > 0)
    {
        int bin = accpos3D->GetBin(binpt, bineta, binphi);
        acccorr = accpos3D->GetBinContent(bin);
    }
    else
    {
        int bin = accneg3D->GetBin(binpt, bineta, binphi);
...
int KshortReconstruction::process_event(PHCompositeNode *topNode)
{
    ...
    m_corrMap.setVerbosity(Verbosity());
    ...
    for (auto tr1_it = m_svtxTrackMap->begin(); tr1_it != m_svtxTrackMap->end(); ++tr1_it)
    {
        ...
        if (tr1->get_quality() > _qual_cut)
        {
            if (Verbosity() > 5)
            {
                std::cout << "Track 1 does not pass quality cut " << tr1->get_quality() << " > cut value " << _qual_cut << std::endl;
            }
            continue;
        }
        // get correction and apply to track pt
        auto [scalecorr_tr1, ringcorr_tr1, etacorr_tr1] = m_corrMap.getCorrections(tr1->get_pt(), tr1->get_eta(), tr1->get_phi(), tr1->get_charge());
        // if any of the corrections are zero, skip this track
        if (scalecorr_tr1 == 0.0 || ringcorr_tr1 == 0.0 || etacorr_tr1 == 0.0)
        {
            if (Verbosity() > 5)
            {
                std::cout << "Track 1 has zero correction factor (as acccorr < 2), skipping" << std::endl;
            }
            continue;
        }
        ...
        if (!corrected_tr1)
        {
            if (Verbosity() > 0)
            {
                std::cout << "Failed to clone track 1" << std::endl;
            }
            continue;
        }
        corrected_tr1->set_px(tr1->get_px() * corrected_pt1 / tr1->get_pt());
        corrected_tr1->set_py(tr1->get_py() * corrected_pt1 / tr1->get_pt());
        corrected_tr1->set_pz(tr1->get_pz()); // pz the same
        if (corrected_pt1 < track_pt_cut)
        {
            if (Verbosity() > 5)
            {
                std::cout << "Track 1 does not pass pt cut " << corrected_pt1 << " < cut value " << track_pt_cut << std::endl;
            }
            continue;
        }

        short int crossing1 = corrected_tr1->get_crossing();

        // calculate number silicon tracks
        double this_dca_cut = track_dca_cut;
        TrackSeed *siliconseed = corrected_tr1->get_silicon_seed();
        if (!siliconseed)
        {
            this_dca_cut *= 5;
            if (Verbosity() > 2)
            {
                std::cout << "silicon seed not found for track 1" << std::endl;
            }
            if (_require_mvtx)
            {
                if (Verbosity() > 5)
                {
                    std::cout << "Track 1 does not pass _require_mvtx" << std::endl;
                }
                continue;
            }
        }
        ...
        if (fabs(dcaVals1(0)) < this_dca_cut || fabs(dcaVals1(1)) < this_dca_cut)
        {
            if (Verbosity() > 5)
            {
                std::cout << "Track 1 does not pass DCA cut " << fabs(dcaVals1(0)) << " or " << fabs(dcaVals1(1)) << " < cut value " << this_dca_cut << std::endl;
            }
            continue;
        }
        ...
        for (auto tr2_it = std::next(tr1_it); tr2_it != m_svtxTrackMap->end(); ++tr2_it)
        {
            ...
            if (tr2->get_quality() > _qual_cut)
            {
                if (Verbosity() > 5)
                {
                    std::cout << "Track 2 does not pass quality cut " << tr2->get_quality() << " > cut value " << _qual_cut << std::endl;
                }
                continue;
            }
            // get correction and apply to track pt
            auto [scalecorr_tr2, ringcorr_tr2, etacorr_tr2] = m_corrMap.getCorrections(tr2->get_pt(), tr2->get_eta(), tr2->get_phi(), tr2->get_charge());
            // if any of the corrections are zero, skip this track
            if (scalecorr_tr2 == 0.0 || ringcorr_tr2 == 0.0 || etacorr_tr2 == 0.0)
            {
                if (Verbosity() > 5)
                {
                    std::cout << "Track 2 has zero correction factor (as acccorr < 2), skipping" << std::endl;
                }
                continue;
            }
            ...
            if (!corrected_tr2)
            {
                if (Verbosity() > 0)
                {
                    std::cout << "Failed to clone track 2" << std::endl;
                }
                continue;
            }
            corrected_tr2->set_px(tr2->get_px() * corrected_pt2 / tr2->get_pt());
            corrected_tr2->set_py(tr2->get_py() * corrected_pt2 / tr2->get_pt());
            corrected_tr2->set_pz(tr2->get_pz());
            if (corrected_pt2 < track_pt_cut)
            {
                if (Verbosity() > 5)
                {
                    std::cout << "Track 2 does not pass pt cut " << corrected_pt2 << " < cut value " << track_pt_cut << std::endl;
                }
                continue;
            }

            short int crossing2 = corrected_tr2->get_crossing();

            // calculate number silicon tracks
            TrackSeed *siliconseed2 = corrected_tr2->get_silicon_seed();
            ...
            if (!siliconseed2)
            {
                ...
                if (Verbosity() > 2)
                {
                    std::cout << "silicon seed not found for track 2" << std::endl;
                }
                if (_require_mvtx)
                {
                    if (Verbosity() > 5)
                    {
                        std::cout << "Track 2 does not pass _require_mvtx" << std::endl;
                    }
                    continue;
                }
            }
            ...
            if (fabs(dcaVals2(0)) < this_dca_cut2 || fabs(dcaVals2(1)) < this_dca_cut2)
            {
                if (Verbosity() > 5)
                {
                    std::cout << "Track 2 does not pass DCA cut " << fabs(dcaVals2(0)) << " or " << fabs(dcaVals2(1)) << " < cut value " << this_dca_cut2 << std::endl;
                }
                continue;
            }
            ...
            if (Verbosity() > 5)
            {
                std::cout << "Pair Track DCA calculation: " << pair_dca << std::endl;
            }
            ...
            if (abs(pair_dca) < pair_dca_cut)
            {
                ...
                if (!ret1 || !ret2)
                {
                    if (Verbosity() > 5)
                    {
                        std::cout << "Could not project one of the tracks to pair PCA point" << std::endl;
                    }
                    continue;
                }
...
void KshortReconstruction::fillNtp(SvtxTrack *track1, SvtxTrack *track2, Acts::Vector3 dcavals1, Acts::Vector3 dcavals2, Acts::Vector3 pca_rel1, Acts::Vector3 pca_rel2, double pair_dca, double invariantMass, double invariantPt, float invariantPhi, float rapidity, float pseudorapidity, Eigen::Vector3d projected_pos1, Eigen::Vector3d projected_pos2, Eigen::Vector3d projected_mom1,
                                   Eigen::Vector3d projected_mom2, Acts::Vector3 pca_rel1_proj, Acts::Vector3 pca_rel2_proj, double pair_dca_proj, unsigned int track1_silicon_cluster_size, unsigned int track2_silicon_cluster_size, unsigned int track1_mvtx_cluster_size, unsigned int track1_mvtx_state_size, unsigned int track1_intt_cluster_size, unsigned int track1_intt_state_size,
                                   unsigned int track2_mvtx_cluster_size, unsigned int track2_mvtx_state_size, unsigned int track2_intt_cluster_size, unsigned int track2_intt_state_size, int runNumber, int eventNumber)
{
    ...
    float reco_info[] = {(float)track1->get_id(),
                         (float)track1->get_crossing(),
                         track1->get_x(),
                         track1->get_y(),
                         track1->get_z(),
                         track1->get_px(),
                         ...
                         (float)pca_rel1(2),
                         (float)eta1,
                         (float)track1->get_charge(),
                         (float)tpcClusters1,
                         (float)track2->get_id(),
                         (float)track2->get_crossing(),
                         track2->get_x(),
                         track2->get_y(),
                         track2->get_z(),
                         track2->get_px(),
                         ...
                         rapidity,
                         pseudorapidity,
                         (float)projected_pos1(0),
                         (float)projected_pos1(1),
                         (float)projected_pos1(2),
                         (float)projected_pos2(0),
                         (float)projected_pos2(1),
                         (float)projected_pos2(2),
                         (float)projected_mom1(0),
                         (float)projected_mom1(1),
                         (float)projected_mom1(2),
                         (float)projected_mom2(0),
                         (float)projected_mom2(1),
                         (float)projected_mom2(2),
                         (float)pca_rel1_proj(0),
                         (float)pca_rel1_proj(1),
                         (float)pca_rel1_proj(2),
                         (float)pca_rel2_proj(0),
                         (float)pca_rel2_proj(1),
                         (float)pca_rel2_proj(2),
                         (float)pair_dca_proj,
                         (float)pathLength_proj(0),
                         (float)pathLength_proj(1),
                         (float)pathLength_proj(2),
                         mag_pathLength_proj,
                         track1->get_quality(),
                         track2->get_quality(),
                         cos_theta_reco,
                         (float)track1_silicon_cluster_size,
                         (float)track2_silicon_cluster_size,
                         (float)track1_mvtx_cluster_size,
                         (float)track1_mvtx_state_size,
                         (float)track1_intt_cluster_size,
                         (float)track1_intt_state_size,
                         (float)track2_mvtx_cluster_size,
                         (float)track2_mvtx_state_size,
                         (float)track2_intt_cluster_size,
                         (float)track2_intt_state_size,
                         (float)runNumber,
                         (float)eventNumber};
...
bool KshortReconstruction::projectTrackToCylinder(SvtxTrack *track, double Radius, Eigen::Vector3d &pos, Eigen::Vector3d &mom)
{
    ...
    std::shared_ptr<Acts::CylinderSurface> cylSurf = Acts::Surface::makeShared<Acts::CylinderSurface>(transform, Radius, halfZ);
    ActsPropagator actsPropagator(_tGeometry);
    auto params = actsPropagator.makeTrackParams(track, m_vertexMap);
    if (!params.ok())
    {
...
int KshortReconstruction::InitRun(PHCompositeNode *topNode)
{
    ...
    ntp_reco_info = new TNtuple("ntp_reco_info", "decay_pairs",
                                "id1:crossing1:x1:y1:z1:px1:py1:pz1:dca3dxy1:dca3dz1:phi1:pca_rel1_x:pca_rel1_y:pca_rel1_z:eta1:charge1:tpcClusters_1:id2:crossing2:x2:y2:z2:px2:py2:pz2:dca3dxy2:dca3dz2:phi2:pca_rel2_x:pca_rel2_y:pca_rel2_z:eta2:charge2:tpcClusters_2:vertex_x:vertex_y:vertex_z:pair_dca:invariant_mass:invariant_pt:invariantPhi:pathlength_x:pathlength_y:pathlength_z:pathlength:"
                                "rapidity:pseudorapidity:projected_pos1_x:projected_pos1_y:projected_pos1_z:projected_pos2_x:projected_pos2_y:projected_pos2_z:projected_mom1_x:projected_mom1_y:projected_mom1_z:projected_mom2_x:projected_mom2_y:projected_mom2_z:projected_pca_rel1_x:projected_pca_rel1_y:projected_pca_rel1_z:projected_pca_rel2_x:projected_pca_rel2_y:projected_pca_rel2_z:projected_"
                                "pair_dca:projected_pathlength_x:projected_pathlength_y:projected_pathlength_z:projected_pathlength:quality1:quality2:cosThetaReco:track1_silicon_clusters:track2_silicon_clusters:track1_mvtx_clusters:track1_mvtx_states:track1_intt_clusters:track1_intt_states:track2_mvtx_clusters:track2_mvtx_states:track2_intt_clusters:track2_intt_states:runNumber:eventNumber");

    if (!m_file_accposneg3D.empty() && !m_file_ptcorr2D.empty() && !m_file_ringcorr2D.empty() && !m_file_etacorr1D.empty())
    {
        m_corrMap.loadCorrMaps(m_file_accposneg3D, m_file_ptcorr2D, m_file_ringcorr2D, m_file_etacorr1D);
    }
    else if (Verbosity() > 0)
    {
        std::cout << "Correction file names not set; skipping corrMap load" << std::endl;
    }
    ...
    if (m_save_tracks)
    {
        ...
        if (Verbosity() > 1)
        {
            std::cout << m_output_trackMap_node_name << " node added" << std::endl;
        }
    }
